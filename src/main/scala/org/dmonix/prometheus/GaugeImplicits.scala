...
trait GaugeImplicits {
  ...
  private[prometheus] object GaugeFunctions {
    /**
     ...
     * @since 1.0
     */
    def autoIncDec[T](preFunc: => Unit, postFunc: => Unit)(block: => T): T = {
      preFunc
      try {
        block
      } finally {
     ...
     * @since 1.0
     */
    def autoIncDecAsync[T](preFunc: => Unit, postFunc: => Unit)(block: => Future[T])(implicit ec:ExecutionContext): Future[T] = {
      preFunc
  ...
  implicit class GaugeDecorator(underlying:Gauge) {
     ...
     */
    def measure[T](block: => T): T = GaugeFunctions.autoIncDec(underlying.inc(), underlying.dec())(block)
     ...
     */
    def measureAsync[T](block: => Future[T])(implicit ec:ExecutionContext): Future[T] = GaugeFunctions.autoIncDecAsync(underlying.inc, underlying.dec)(block)
